...
###############setup pamameter###################
Com = "COM12"
c = 'TCT result-{}.xlsx'.format(time.strftime("%Y-%m-%d %H%M%S", time.localtime()))
...
ws_rx = wb.create_sheet("Rx")
dpa_col_offs = len(branches) * len(mpa_index) * 4
for b in branch_index:
    for i in range(len(mpa_index)):
        ws_pa.cell(row=1, column=b * len(mpa_index) * 4 + i * 4 + 1).value = 'MPA {}.{}'.format(b, i)
        ws_pa.cell(row=2, column=b*len(mpa_index) * 4 + i * 4 + 1).value = 'Temp'
        ws_pa.cell(row=2, column=b*len(mpa_index) * 4 + i * 4 + 2).value = 'Offset'
    for i in range(len(dpa_index)):
        ws_pa.cell(row=1, column=dpa_col_offs + b*len(dpa_index) * 4 + i * 4 + 1).value = 'DPA {}.{}'.format(b, i)
        ws_pa.cell(row=2, column=dpa_col_offs + b*len(dpa_index) * 4 + i * 4 + 1).value = 'Temp'
        ws_pa.cell(row=2, column=dpa_col_offs + b*len(dpa_index) * 4 + i * 4 + 2).value = 'Offset'
    ws_tx.cell(row=1, column=b * 4 + 1).value = 'Tx {}'.format(b)
    ws_tx.cell(row=2, column=b * 4 + 1).value = 'Temp'
    ws_tx.cell(row=2, column=b * 4 + 2).value = 'Power'
    ws_rx.cell(row=1, column=b * 10 + 1).value = 'Rx {}'.format(b)
    ws_rx.cell(row=2, column=b * 10 + 1).value = 'Temp'
    ws_rx.cell(row=2, column=b * 10 + 2).value = 'HighGainNoAtt'
    ws_rx.cell(row=2, column=b * 10 + 3).value = 'HighGainAtt'
    ws_rx.cell(row=2, column=b * 10 + 4).value = 'BypassNoAtt'
    ws_rx.cell(row=2, column=b * 10 + 5).value = 'BypassAtt'
...
while time.time() - start_time < time_total:
    ...
    for b in branch_index:
        ...
        rx_temp[b].append(float(ru.get_single_data('ts r Rx:{}'.format(branches[b]))))
        sg.set_rf_state('ON')
        ru.set_rx_front_end(branch=branches[b], bypass='off', att='off')
        time.sleep(0.5)
        rx_highgain_noatt[b].append(ru.get_rxipwr_fb(fb = rx_ipwr_FB[b], repeat = 10))
        ru.set_rx_front_end(branch=branches[b], bypass='off', att='on')
        time.sleep(0.5)
        rx_highgain_att[b].append(ru.get_rxipwr_fb(fb = rx_ipwr_FB[b], repeat = 10))
        ru.set_rx_front_end(branch=branches[b], bypass='on', att='off')
        time.sleep(0.5)
        rx_bypass_noatt[b].append(ru.get_rxipwr_fb(fb = rx_ipwr_FB[b], repeat = 10))
        ru.set_rx_front_end(branch=branches[b], bypass='on', att='on')
        time.sleep(0.5)
        rx_bypass_att[b].append(ru.get_rxipwr_fb(fb = rx_ipwr_FB[b], repeat = 10))
        sg.set_rf_state('OFF')
        ws_rx.cell(row=count + 3, column=b * 10 + 1).value = rx_temp[b][count]
        ws_rx.cell(row=count + 3, column=b * 10 + 2).value = rx_highgain_noatt[b][count]
        ws_rx.cell(row=count + 3, column=b * 10 + 3).value = rx_highgain_att[b][count]
        ws_rx.cell(row=count + 3, column=b * 10 + 4).value = rx_bypass_noatt[b][count]
        ws_rx.cell(row=count + 3, column=b * 10 + 5).value = rx_bypass_att[b][count]
    ##########PA cal##########################
    for i in vdd_offset:
    ...
    for b in branch_index:
        ...
        ####final PA#####
        for i in range(len(mpa_index)):
            mpa_offset[b][i].append(ru.calibrate_pa(name='Mpa:{}.{}'.format(b, mpa_index[i]), target=mpa_target[i],start_offset=mpa_start_offset[i], accurecy=5))
            pa_final_temp[b][i].append(float(ru.get_single_data('ts r PaFinal:{}'.format(branches[b]))))
            ws_pa.cell(row=count + 3, column=b * len(mpa_index) * 4 + i * 4 + 1).value = pa_final_temp[b][i][count]
            ws_pa.cell(row=count + 3, column=b * len(mpa_index) * 4 + i * 4 + 2).value = mpa_offset[b][i][count]
        ####driver PA#####
        for i in range(len(dpa_index)):
            dpa_offset[b][i].append(ru.calibrate_pa(name='Dpa:{}.{}'.format(b, dpa_index[i]), target=dpa_target[i],start_offset=dpa_start_offset[i], accurecy=2))
            pa_driver_temp[b][i].append(float(ru.get_single_data('ts r PaDriver:{}'.format(branches[b]))))
            ws_pa.cell(row=count + 3, column=dpa_col_offs + b * len(dpa_index) * 4 + i * 4 + 1).value = pa_driver_temp[b][i][count]
            ws_pa.cell(row=count + 3, column=dpa_col_offs + b * len(dpa_index) * 4 + i * 4 + 2).value = dpa_offset[b][i][count]
    ...
    for b in branch_index:
        ...
        tor_temp[b].append(float(ru.get_single_data('ts r Tor:{}'.format(branches[b]))))
        trdc_tx.trdc_release_all()
        ws_tx.cell(row=count + 3, column=b * 4 + 1).value = tor_temp[b][count]
        ws_tx.cell(row=count + 3, column=b * 4 + 2).value = tor_outputpower[b][count]
