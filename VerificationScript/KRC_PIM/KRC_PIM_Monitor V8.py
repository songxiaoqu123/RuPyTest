...
def pim_test(dut_sn):
    ...
    #############Read rx power with Tx1 ON TX2 ON############
    ...
    time.sleep(5)
    messagebox.showinfo("Knocking", "Please Click OK and Knock the FU\nXin hãy nhấn OK và bắt đầu gõ cửa")
    ...
    while curret_time - monitor_start_time < monitor_time:
        Rx_temp = rru.sendWait('ts r Rx:A')[12:-6]
        rx_ipwr = rru.get_rxipwr()
        A_Rx1 = round(rx_ipwr[0]-rx_gain, 2)
        A_Rx2 = round(rx_ipwr[1]-rx_gain, 2)
        ...
        if curret_time - monitor_start_time > 10 and tapping_flag ==1:
            tapping_flag = 0
            with open(log_file_name, 'a') as f:
                f.write('Tapping finished\n\n\n')
                f.write('{:<30}{:<20}{:<20}{:<20}{:<20}{:<20}\n'.format('Time', 'RxTemp', 'A_RX1', 'A_RX2', 'B_RX1', 'B_RX2'))
        with open(log_file_name, 'a') as f:
            f.write('{:<30}{:<20}{:<20}{:<20}{:<20}{:<20}\n'.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),Rx_temp,A_Rx1, A_Rx2, B_Rx1, B_Rx2))
        if A_Rx1 - A_Rx1_Tx_00_ave > 1 or A_Rx2 - A_Rx2_Tx_00_ave > 1 or B_Rx1 - B_Rx1_Tx_00_ave > 1 or B_Rx2 - B_Rx2_Tx_00_ave > 1:
            fail_flag = True
            break
    ...
    rru.sendWait('trdc off 2')
    rru.sendWait('trdc off 3')
    rru.sendWait('trdc off 4')
    rru.sendWait('trdc off 5')
    rru.sendWait('trdc off 6')
    rru.sendWait('trdc off 7')
    rru.sendWait('trdc off 8')
    ...
    rru.closeCom()
    ########################Shut down PSU##############
    psu.set_output(state='OFF', before_delay=1, after_delay=1)
    ##########Limit cal and check################
    ...
    if fail_flag:
        ...
        tk.Label(pop_window, text='FAIL', font=12, bg='red',width = 100,height = 100).pack(side='top')
        ...
        pop_window.title('Result')  # window name
        pop_window.geometry('300x300')  # window size
        tk.Label(pop_window, text='PASS', font=12, bg='green',width = 100,height = 100).pack(side='top')
        tk.Button(pop_window, text='OK', font=12, command=pop_window.destroy).pack(side='bottom')
        pop_window.mainloop()
