...
current_row = 1
current_col = 1
dut_data = ['time', 'Sample']
vsa_data = ['Power', 'ACLR1_L', 'ACLR1_R','ACLR2_L','ACLR2_R', 'EVM_rms', 'Freq_err', 'IQ_offset']
for i in dut_index:
        ...
        for i in dut_index:
            ...
            for j in port_index:
                ...
                if vsa:
                    ...
                    for d in acp_key:
                        ws[i].cell(row=current_row, column=current_col).value = acp_data[d]
                        current_col += 1
                    vsa.send('*RCL 2')
                    vsa.set_freq_center(tx_freq)
                    vsa.set_ext_gain_BTS(tx_pathloss[test_index])
                    ...
                    for d in evm_key:
                        ws[i].cell(row=current_row, column=current_col).value = evm_data[d]
                        current_col += 1
