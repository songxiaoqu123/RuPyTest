...
temp_list = [50, 50, 50, 50]
cycle = 0
#########StartTest###################################################
while True:
    try:
        if ((temp_list[0] < lowtemp) and (temp_list[2] < lowtemp)) and (state == 'Cooling'):
            state = 'Heating'
            for i in dut_index:
        ...
        if ((temp_list[0] > hightemp) and (temp_list[2] > hightemp)) and (state == 'Heating'):
            state = 'Cooling'
            for i in dut_index:
        ...
        for i in dut_index:
            for j in port_index:
                ...
                dut[i].openCom()
###########
                temp = dut[i].sendWait('ts r DcDpaVdd:0')
                temp = temp[18:-6]
                temp_list[test_index] = float(temp)
###################
