...
class TxRfFuncConfig:
    ...
    def write_carrier_combination(self, worksheet_out, carrier_comb_narrow, carrier_comb):
        bw = []
        carrier_name = []
        carrier_testmod = []
        carrier_backoff = []
        ...
        if self.close_carrier and sum(bw) + self.close_gap * (len(carrier_comb) - 1) < self.full_bw:
    ...
    def gen_carrier_combination(self):
        ...
        for i1 in range(len(carrier_setup)):
            ...
            current_row += 1
            worksheet_out.cell(row=current_row, column=current_col).value = '{}dB'.format(carrier_setup[i1]['carrier_backoff'])
            current_row += 1
        ...
        for i1 in range(len(carrier_setup)):
            for i2 in range(len(carrier_setup)):
                ...
                if i2 >= i1 and self.check_carrier_combin_availible(carrier_comb):
                    self.write_carrier_combination(worksheet_out, carrier_comb_narrow, carrier_comb)
        self.workbook.save(self.config_file_add)
                    ...
                    for i4 in range(len(carrier_setup)):
                        for i5 in range(len(carrier_setup)):
                            for i6 in range(len(carrier_setup)):
                                ...
                                if i2 >= i1 and i3 >=i2 and i4>=i3 and i5>=i4 and i6>=i5 and self.check_carrier_combin_availible(carrier_comb):
                                    self.write_carrier_combination(worksheet_out, carrier_comb_narrow, carrier_comb)
        self.workbook.save(self.config_file_add)
