...
def pim_test(dut_sn):
    ...
    time.sleep(3)
    A_tx_power_10 = float(rru.get_trxpower(0)[1].strip(','))
    B_tx_power_10 = float(rru.get_trxpower(1)[1].strip(','))
    with open(log_file_name, 'a') as f:
        f.write('TX1 ON, TX2 OFF\n')
        f.write('TX_power_A={:<20} TX_power_B={:<20}\n'.format(A_tx_power_10, B_tx_power_10))
        f.write('{:<20}{:<20}{:<20}{:<20}\n'.format('A_RX1', 'A_RX2', 'B_RX1', 'B_RX2'))
    for i in range(repeat_times):
        rx_ipwr = rru.get_rxipwr()
        A_Rx1 = rx_ipwr[0]-rx_gain
        ...
        B_Rx1 = rx_ipwr[12]-rx_gain
        B_Rx2 = rx_ipwr[13]-rx_gain
        A_Rx1_Tx_10.append(A_Rx1)
        A_Rx2_Tx_10.append(A_Rx2)
        B_Rx1_Tx_10.append(B_Rx1)
        B_Rx2_Tx_10.append(B_Rx2)
        with open(log_file_name, 'a') as f:
            f.write('{:<20}{:<20}{:<20}{:<20}\n'.format(A_Rx1, A_Rx2, B_Rx1, B_Rx2))
        time.sleep(delay_per_sample)
    A_Rx1_Tx_10_ave = np.array(A_Rx1_Tx_10).mean()
    A_Rx2_Tx_10_ave = np.array(A_Rx2_Tx_10).mean()
    B_Rx1_Tx_10_ave = np.array(B_Rx1_Tx_10).mean()
    B_Rx2_Tx_10_ave = np.array(B_Rx2_Tx_10).mean()
    with open(log_file_name, 'a') as f:
        f.write('\n{:<20}{:<20}{:<20}{:<20}\n'.format('A_RX1_AVE', 'A_RX2_AVE', 'B_RX1_AVE', 'B_RX2_AVE'))
        f.write('{:<20}{:<20}{:<20}{:<20}\n'.format(A_Rx1_Tx_10_ave, A_Rx2_Tx_10_ave, B_Rx1_Tx_10_ave, B_Rx2_Tx_10_ave))
        f.write('\n\n')
    ...
    if (abs(A_tx_power_10+9.7)>1):
        with open(log_file_name, 'a') as f:
            f.write('A TX1 POWER FAIL\n')
            fail_flag = True

    if (abs(B_tx_power_10+9.7)>1):
        with open(log_file_name, 'a') as f:
            f.write('B TX1 POWER FAIL\n')
            fail_flag = True

    if (abs(A_tx_power_11+8.7)>1):
        with open(log_file_name, 'a') as f:
            f.write('A TX ALL POWER FAIL\n')
            fail_flag = True

    if (abs(B_tx_power_11+8.7)>1):
        with open(log_file_name, 'a') as f:
            f.write('B TX ALL POWER FAIL\n')
            fail_flag = True
    ...
    if fail_flag:
        ...
        with open('fail_temp.txt', 'w')as f:
            f.write('FAIL')
    else:
        f.write('PASS')
    time.sleep(5)
...
def get_fail_flag():
    with open('fail_temp.txt', 'r') as f:
        msg = f.read()
        return msg
def refreshText():
    text.set(get_fail_flag())
...
text = tk.StringVar()
text.set(get_fail_flag())
result_lable = tk.Label(root, textvariable=text, font=font)
