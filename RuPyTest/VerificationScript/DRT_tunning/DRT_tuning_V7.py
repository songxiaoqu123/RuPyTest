...
carriers_setup_string = ['L50tm3p1_755.5e6_P-3.0dB','L50tm3p1_788.5e6_P-3.0dB']
        ...
        for j in range(len(drt.DRT_index)): #DRT中需要优化的lut个数
            for i2 in range(iter_lut):    # 每个lut迭代次数
                for d_a in range(2):   #0 is a_dalay and 1 is d_daley
                    drt.obue_margin_best=-99        #每一次重新迭代D或者A delay的时候，都重新寻找最佳值。
                    for delay in drt.DRT_range:   #优化a_delay
                        ...
                        if drt.check_delay_available():
                            ...
                            if min(spur_vsa) > drt.obue_margin_best:
                                ...
                                if drt.obue_margin_best > margin_target:
                                    pass
...
with open('DRT log.txt', 'a') as f:
    f.write('\n\n')
    for j in range(len(drt.delay_best)):
        f.write('db write {}{}/xLut{}  0 {} {} 0 0\n'.format(drt.db_prefix, drt.section, drt.DRT_index[j], drt.delay_inuse[j][0], drt.delay_inuse[j][1]))
